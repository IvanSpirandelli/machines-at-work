...
#!/usr/bin/env bash
FAKE_DIR="$TMP" SCAFFOLD="$SCAFFOLD" exec bash "$TMP/fake-claude-body.sh"
EOF
...
if [ "$n" -eq 1 ]; then
  ...
  "$SCAFFOLD/scripts/task.sh" start 0001 >/dev/null
...
"$SCAFFOLD/scripts/task.sh" start 0001 >/dev/null    # resume in-progress
git -C app add -A
git -C app -c user.email=t@t -c user.name=t commit -qm "finish work" 2>/dev/null || true
"$SCAFFOLD/scripts/task.sh" done 0001 >/dev/null
echo '{"total_cost_usd": 0.05}'
EOF
chmod +x "$TMP/bin/claude"

cd "$WS"
"$SCAFFOLD/scripts/task.sh" new "Limit retry feature" >/dev/null

out=$(PATH="$TMP/bin:$PATH" LIMIT_BACKOFF=2 MAX_TASKS=3 bash "$SCAFFOLD/scripts/loop.sh" 2>&1) || fail "loop.sh exited nonzero: $out"
...
grep -q "Status: done" scaffold/tasks/0001-*/task.md || fail "task not done after retry"
[ ! -s scaffold/NEEDS_HUMAN.md ] || fail "task was escalated: $(cat scaffold/NEEDS_HUMAN.md)"
